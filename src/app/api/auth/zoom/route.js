...
export async function GET(request) {
  try {
    ...
    const state = Buffer.from(JSON.stringify({ instructorId })).toString(
      "base64"
    );

    const zoomAuthUrl = new URL(
      "https://zoom.us/oauth/authorize?response_type=code&client_id=3T9tCxvnR0W_PP_BWsoXUw&redirect_uri=https://lms-chi-blue.vercel.app/api/auth/zoom/callback"
    );
    ...
    zoomAuthUrl.searchParams.append(
      "scope",
